...
typedef struct _tracker_queue_s
{
    ...
    struct request_queue*    original_queue;  // 跟踪的设备的原始请求队列，
                                              // （内核就提供一种队列的机制把这些I/O请求添加到队列中(即：请求队列)，在驱动中用request_queue结构体描述）
    make_request_fn*        original_make_request_fn;  // 跟踪的设备的原始make_request_fn函数
                                                       // （设备将一个新请求插入请求队列时调用的方法，块设备的io调度程序主要是在该函数内完成）
